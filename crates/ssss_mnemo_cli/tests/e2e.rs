...
fn run_split(encoding: &str, k: u8, n: u8, input: &[u8]) -> Vec<String> {
...
fn run_combine(from: &str, shares: &[String]) -> Vec<u8> {
    let stdin = shares.join("\n") + "\n";

    let mut cmd = Command::new(assert_cmd::cargo::cargo_bin!("ssss_mnemo_cli"));
    let assert = cmd
        .args(["combine", "--from", from, "--in-stdin", "--out-stdout"])
        .write_stdin(stdin)
        .assert()
        .success();
...
fn e2e_round_trip_base58check() {
    ...
    let shares = run_split("base58check", 2, 3, input);
    let recovered = run_combine("base58check", &shares[..2]);
...
fn e2e_round_trip_base64url() {
    ...
    let recovered = run_combine("base64url", &shares[..2]);
    assert_eq!(recovered, input);
}
...
fn e2e_round_trip_mnemo_words() {
    ...
    let shares = run_split("mnemo-words", 2, 3, input);
    let recovered = run_combine("mnemo-words", &shares[..2]);
    assert_eq!(recovered, input);
}
