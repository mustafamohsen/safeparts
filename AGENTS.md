...
- There is a web app under `web/` (Vite + React) which expects a WASM build step.
- Help/docs live under `web/help/` (Astro + Starlight), deployed under `/help/`.
- CI workflows:
  - `.github/workflows/rust-ci.yml`: `cargo fmt` / `cargo clippy` / `cargo test`
  - `.github/workflows/web-ci.yml`: build WASM + web + docs, then Playwright a11y tests
  - `.github/workflows/release.yml`: build + publish archives on tags `v*`
...
- Playwright + axe a11y tests live under `web/tests/`.
  - Install browser: `bun run test:a11y:install`
