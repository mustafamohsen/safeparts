...
Safeparts treats the input as a byte array and splits it **byte-by-byte**. The split input is:
...
Each share payload is the concatenation of its `y` values across all `L` positions, so every share has the same length as the tagged payload. The share packet stores its own `x` value, along with `k`, `n`, and `set_id`, so reconstruction can pick any <var>k</var> shares and interpolate each byte independently.
